...
import {
  ...
  amdImportId,
} from "@arijs/babel-helper-module-transforms";
...
export default declare((api, options) => {
  ...
  function buildBrowserArg(
    ...
    if (exactGlobals) {
      const globalRef =
        (sourceFilename && browserGlobals[sourceFilename]) ||
        browserGlobals[source];
      if (globalRef) {
      ...
      Program: {
        exit(path) {
          ...
          for (const [source, metadata] of meta.source) {
            ...
            const sourceExt = withExtension(source, importFileExt);
            commonjsArgs.push(
              t.callExpression(t.identifier("require"), [
                t.stringLiteral(sourceExt),
              ]),
            );
            browserArgs.push(
              buildBrowserArg(
